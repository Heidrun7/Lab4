...
// We only want numbers other than 0 where i=j
function p4(mat) {
    ...
    for(var i=0; i < mat.length; i++) {
        for(var j=0; j < mat.length; j++) {
            if(i !== j && mat[i][j] !== 0) { // If i and j are not the same index and the number is not 0
                return false
            }
        }
    }
...
function p8() {
    var currImg = document.getElementById("p8img").src; // Get the current image, so the random one won't be the same
    var imgArr = [
    ...
    while (currImg === imgArr[randNum]) { // Loop until the current image is not the same as the new one
